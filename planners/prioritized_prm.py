...
class PriorityPrm():
    def __init__(self, robots, env, goals):
        ...
        self.roadmap = ManhattanRoadmap(
            self.robots, self.env, self.goalLocs, self.N_SAMPLE, self.N_KNN, self.MAX_EDGE_LEN, self.GRID_SPACING)
        # self.plot_roadmap()
        self.constraintSets = self.ConstraintSets(
            self.robots, self.env, self.roadmap)
    ...
    def astar_planning(self, cur_robot_id, useTime):
        ...
        startNode = self.Node(
            ...
            cost=0.0, pind=-1, timestep=0, index=start_id, useTime=useTime)
        goalNode = self.Node(
            self.goalLocs[cur_robot_id], cost = 0.0, pind=-1, timestep=-1, index=goal_id, useTime=useTime)
        ...
        while True:
            ...
            curKey=min(
                openSet, key=lambda o: openSet[o].cost + self.calc_heuristic(openSet[o], goalNode, useTime))
            ...
            if self.node_is_valid(curNode, cur_robot_id):
                ...
                for i in range(len(conns)):
                    ...
                    newNode=self.Node(loc=newLoc, cost=curNode.cost + 1, pind=curNode.index,
                                        timestep=curNode.timestep+1, index=new_id, useTime=useTime)
        ...
        path_indices=[self.roadmap.get_goal_index(
            cur_robot_id)]  # Add goal node = goalNo
        pkey = goalNode.pkey
        pind=goalNode.pind
        while pind != -1:
    ...
    class ConstraintSets:
        ...
        def get_rigid_subset(self, trajs, cur_robot_id, timestep, original_set):
            rigid_subset=set([loc_id for loc_id in original_set if self.state_would_be_rigid(
                trajs, cur_robot_id, timestep, loc_id)])
            return rigid_subset
        ...
        def animate_rigid_states(self, cur_robot_id, coordTrajs, goalLocs):
            ...
            for timestep in range(maxTimestep):
                ...
                plot.plot(self.robots.get_robot_graph(), self.env, blocking=False,
                          animation=True, clear_last=False, show_graph_edges=True)
                # if timestep == 0:
                #     plt.pause(10)
            plt.close()
