...
class BaseExprVisitor(ast.NodeVisitor):
    ...
    def _rewrite_membership_op(self, node, left, right):
        ...
        if is_term(left) and is_term(right) and op_type in self.rewrite_map:
            ...
            if right_str:
                name = self._env.add_tmp([right.value])
                right = self.term_type(name, self._env)

            if left_str:
                name = self._env.add_tmp([left.value])
                left = self.term_type(name, self._env)
    ...
    def _maybe_downcast_constants(self, left, right):
        ...
        if (
            ...
            name = self._env.add_tmp(np.float32(left.value))
            left = self.term_type(name, self._env)
        if (
            right.is_scalar
            ...
            name = self._env.add_tmp(np.float32(right.value))
            right = self.term_type(name, self._env)
    ...
    def _maybe_eval(self, binop, eval_in_python):
        ...
        return binop.evaluate(
            self._env, self.engine, self.parser, self.term_type, eval_in_python
        )
    ...
    def visit_Name(self, node, **kwargs):
        return self.term_type(node.id, self._env, **kwargs)

    def visit_NameConstant(self, node, **kwargs):
        return self.const_type(node.value, self._env)

    def visit_Num(self, node, **kwargs):
        return self.const_type(node.n, self._env)

    def visit_Constant(self, node, **kwargs):
        return self.const_type(node.n, self._env)

    def visit_Str(self, node, **kwargs):
        name = self._env.add_tmp(node.s)
        return self.term_type(name, self._env)

    def visit_List(self, node, **kwargs):
        name = self._env.add_tmp([self.visit(e)(self._env) for e in node.elts])
        return self.term_type(name, self._env)
    ...
    def visit_Subscript(self, node, **kwargs):
        ...
        result = pd.eval(
            slobj, local_dict=self._env, engine=self.engine, parser=self.parser
        )
        try:
            # a Term instance
            ...
            # an Op instance
            lhs = pd.eval(
                value, local_dict=self._env, engine=self.engine, parser=self.parser
            )
            v = lhs[result]
        name = self._env.add_tmp(v)
        return self.term_type(name, env=self._env)
    ...
    def visit_Call(self, node, side=None, **kwargs):
        ...
        if isinstance(res, FuncNode):
            ...
            return self.const_type(res(*new_args, **kwargs), self._env)
...
class Expr:
    ...
    def __init__(
        ...
        self.expr = expr
        self._env = env or Scope(level=level + 1)
        self.engine = engine
        self.parser = parser
        self._visitor = _parsers[parser](self._env, self.engine, self.parser)
