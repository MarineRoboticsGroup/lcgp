...
class Term(ops.Term):
    ...
    def _resolve_name(self):
        ...
        if self.side == "left":
            ...
            if self.name not in self._env.queryables:
                raise NameError(f"name {repr(self.name)} is not defined")
            return self.name

        # resolve the rhs (and allow it to be None)
        try:
            return self._env.resolve(self.name, is_local=False)
...
class PyTablesExprVisitor(BaseExprVisitor):
    ...
    def visit_UnaryOp(self, node, **kwargs):
        if isinstance(node.op, (ast.Not, ast.Invert)):
            ...
            return self.const_type(-self.visit(node.operand).value, self._env)
    ...
    def visit_Subscript(self, node, **kwargs):
        ...
        try:
            return self.const_type(value[slobj], self._env)
    ...
    def visit_Attribute(self, node, **kwargs):
        ...
        if ctx == ast.Load:
            ...
            try:
                return self.term_type(getattr(resolved, attr), self._env)
            except AttributeError:
...
class PyTablesExpr(expr.Expr):
    ...
    def __init__(
        ...
        self.expr = _where
        self._env = PyTablesScope(scope_level + 1, local_dict=local_dict)

        if queryables is not None and isinstance(self.expr, str):
            self._env.queryables.update(queryables)
