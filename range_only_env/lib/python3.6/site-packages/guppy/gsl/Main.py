...
class Description:
    ...
    def resolve_aspects(self):
        self.init_localview()
        if self.src.args:
            self.args = [self._env.get_descr_by_subject(
...
class Package(Description):
    ...
    def import_package(self, name, context):
        pac = self.imported_packages.get(name)
        if pac is None:
            pac = self._env.import_package(name, context)
            self.imported_packages[name] = pac
        return pac
...
class ConditionRef(Description):
    ...
    def resolve_special(self):
        ...
        self.cond_definition = self._env.get_descr_by_subject(cond_def)
