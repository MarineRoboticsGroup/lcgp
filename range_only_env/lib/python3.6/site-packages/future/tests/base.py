...
class CodeHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if pypath:
            self._env = {'PYTHONPATH': os.getcwd() + os.pathsep + pypath}
        else:
            self._env = {'PYTHONPATH': os.getcwd()}
    ...
    def _futurize_test_script(self, filename='mytestscript.py', stages=(1, 2),
        ...
        try:
            output = check_output(call_args, stderr=STDOUT, env=self._env)
