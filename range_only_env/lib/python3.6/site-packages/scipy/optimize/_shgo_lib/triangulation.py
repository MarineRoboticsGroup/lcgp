    ...
    def plot_complex(self):
        ...
        if self.dim == 2:
                    ...
                    for v in c():
                        ...
                        for vn in v.nn:
                            if self._bounds is None:
                                ...
                                xn_a = np.array(vn.x, dtype=float)
                                for i in range(len(self._bounds)):
                                    xn_a[i] = (xn_a[i] * (self._bounds[i][1]
                                                          - self._bounds[i][0])
                                               + self._bounds[i][0])
            ...
            if self._bounds is None:
                ...
                pyplot.ylim(
                    [self._bounds[1][0] - 1e-2, self._bounds[1][1] + 1e-2])
                pyplot.xlim(
                    [self._bounds[0][0] - 1e-2, self._bounds[0][1] + 1e-2])
...
class VertexCache:
    ...
    def __getitem__(self, x, indexed=True):
        try:
            ...
            if indexed:
                self.index += 1
                xval = Vertex(x, bounds=self._bounds,
