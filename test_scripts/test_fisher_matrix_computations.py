...
if __name__ == '__main__':
    ...
    loc_lists = [generate_loc_list(num_locs, xlb, xub, ylb, yub) for x in range(num_repeats)]
    graph_list = []

    for i, loc_list in enumerate(loc_lists):
        testGraph = graph.Graph(noise_model, noise_stddev)
        testGraph.initialize_from_location_list(loc_list, sensing_radius)
        graph_list.append(testGraph)
